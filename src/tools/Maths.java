...
public class Maths {
    ...
    public static Matrix4f createModelMatrix(Vector3f translation, float rx, float ry, float rz, float scale) {
        ...
        matrix = matrix.scale(new Vector3f(scale, scale, scale));
        return matrix;
    }
    ...
    public static Matrix4f createViewMatrix(Camera camera) {
        ...
        viewMatrix = viewMatrix.rotate((float) Math.toRadians(camera.getPitch()), new Vector3f(1, 0, 0));
        viewMatrix = viewMatrix.rotate((float) Math.toRadians(camera.getYaw()), new Vector3f(0, 1, 0));
