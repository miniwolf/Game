...
vec3 calculatePointLight(const PointLight light){
    const vec3 direction = normalize(light.position - frag.worldPositionFrag);
    const float distanceToLight = distance(light.position, frag.worldPositionFrag);
    const float attenuation = attenuate(distanceToLight);
    const float d = max(dot(normalize(frag.normalFrag), direction), 0.0f);
...
void main(){
    ...
    if (!isInsideCube(frag.worldPositionFrag, 0)) {
        return;
    }
    ...
    vec3 voxel = scaleAndBias(frag.worldPositionFrag);
