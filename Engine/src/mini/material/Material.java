...
public class Material implements CloneableSmartAsset, Cloneable {
    ...
    public int contentHashCode() {
        ...
        hash = 29 * hash + (this.technique != null ? this.technique.getDef().getName().hashCode() :
                            0);
        hash = 29 * hash + (this.additionalState != null ? this.additionalState.contentHashCode() :
                            0);
        return hash;
    }
    ...
    private void checkSetParam(VarType type, String name) {
        ...
        if (type != null && paramDef.getVarType() != type) {
            System.err.println("Material parameter being set: " + name + " with "
                               + "type " + type.name() + " doesn''t match definition types "
                               + paramDef.getVarType());
        }
    }
    ...
    private void checkTextureParamColorSpace(String name, Texture value) {
        ...
        if (paramDef.getColorSpace() != null && paramDef.getColorSpace() != value.getImage()
                                                                                 .getColorSpace()) {
            value.getImage().setColorSpace(paramDef.getColorSpace());
            System.err.println("Material parameter " + name + " needs a "
                               + paramDef.getColorSpace().toString() + " texture, "
                               + "texture " + value.getName() + " was switched to "
                               + value.getImage().getColorSpace().name() + " color space.");
        } else if (paramDef.getColorSpace() == null && value.getName() != null
                   && value.getImage().getColorSpace() == ColorSpace.Linear) {
            System.err.println("The texture " + value.getName()
                               + " has linear color space, but the material "
                               + "parameter " + value.getImage().getColorSpace().name()
                               + " specifies no color space requirement, this may "
                               + "lead to unexpected behavior.\nCheck if the image "
                               + "was not set to another material parameter with a linear "
                               + "color space, or that you did not set the ColorSpace to "
                               + "Linear using texture.getImage.setColorSpace(). " + name);
        }
    }
     ...
     *
     * @param name  the name of the matrix defined in the material definition (minid)
     * @param value the Matrix4f object
     */
    public void setMatrix4(String name, Matrix4f value) {
        setParam(name, VarType.Matrix4f, value);
    }

    /**
     * Pass a boolean to the material shader.
     *
     * @param name  the name of the boolean defined in the material definition (minid)
     ...
     *
     * @param name  the name of the float defined in the material definition (minid)
     ...
     *
     * @param name  the name of the int defined in the material definition (minid)
    ...
    public void setVector2(String name, Vector2f value) {
        setParam(name, VarType.Vector2f, value);
    }
    ...
    public void setVector3(String name, Vector3f value) {
        setParam(name, VarType.Vector3f, value);
    }
    ...
    private void updateRenderState(RenderManager renderManager, Renderer renderer,
                                   TechniqueDef techniqueDef) {
        if (renderManager.getForcedRenderState() != null) {
            renderer.applyRenderState(renderManager.getForcedRenderState());
        } else {
            if (techniqueDef.getRenderState() != null) {
                renderer.applyRenderState(techniqueDef.getRenderState()
                                                      .copyMergedTo(additionalState,
                                                                    mergedRenderState));
            } else {
    ...
    public void render(Geometry geometry, LightList lights, RenderManager renderManager) {
        ...
        int unit = updateShaderMaterialParameters(renderer, shader, overrides,
                                                  renderManager.getForcedMatParams());
    ...
    public String toString() {
        return "Material[name=" + name +
               ...
               ", tech=" + (technique != null && technique.getDef() != null ?
                            technique.getDef().getName() : null) +
               "]";
    }
}
